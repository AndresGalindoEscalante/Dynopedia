...
class Insert
{
    ...
    public function insertar($fecha_creacion, $tipo, $administrador_id)
    {
        // Esta es la consulta sql que inserta en la base de datos segun los id que se les pase por parametro
        $sql = "INSERT INTO pagina VALUES(null, :fechaCreacion, :tipo, :administrador_id)";
        $conexion = self::conectar();
        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':fechaCreacion', $fecha_creacion);
        $stmt->bindParam(':tipo', $tipo);
        $stmt->bindParam(':administrador_id', $administrador_id);
